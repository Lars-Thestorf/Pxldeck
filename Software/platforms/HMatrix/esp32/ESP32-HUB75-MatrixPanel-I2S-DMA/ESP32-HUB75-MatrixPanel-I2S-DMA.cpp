...
bool MatrixPanel_I2S_DMA::allocateDMAmemory()
{
   ...
   */        
    ...
    if(_dma_linked_list_memory_required > heap_caps_get_largest_free_block(MALLOC_CAP_DMA)) {
       printf(("ERROR: Not enough SRAM left over for DMA linked-list descriptor memory reservation! Oh so close!\r\n"));
    ...
    if(!dmadesc_a) {
        printf(("ERROR: Could not malloc descriptor framebuffer a."));
        return false;
    }
    ...
    if (m_cfg.double_buff) // reserve space for second framebuffer linked list
    {
        ...
        if(!dmadesc_b) {
            printf(("ERROR: Could not malloc descriptor framebuffer b."));
            return false;
        }
    }

    printf(("*** ESP32-HUB75-MatrixPanel-I2S-DMA: Memory Allocations Complete ***"));
    printf(("Total memory that was reserved: %d kB.\r\n"), _total_dma_capable_memory_reserved/1024);
    printf(("... of which was used for the DMA Linked List(s): %d kB.\r\n"), _dma_linked_list_memory_required/1024);
	
    printf(("Heap Memory Available: %d bytes total. Largest free block: %d bytes.\r\n"), heap_caps_get_free_size(0), heap_caps_get_largest_free_block(0));
    printf(("General RAM Available: %d bytes total. Largest free block: %d bytes.\r\n"), heap_caps_get_free_size(MALLOC_CAP_DEFAULT), heap_caps_get_largest_free_block(MALLOC_CAP_DEFAULT));
